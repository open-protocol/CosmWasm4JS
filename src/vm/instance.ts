...
export class Instance {
  ...
  public store: Map<string, Buffer>;
  ...
  public static async fromCode(code: Buffer, store: Map<string, Buffer>, api: BackendApi, querier: Querier): Promise<Instance> {
  ...
  static fromModule(module: WebAssembly.Module, memory: WebAssembly.Memory, store: Map<string, Buffer>, api: BackendApi, querier: Querier): Instance {
    const instance = new Instance(new WebAssembly.Instance(module, {
      env: {
        ...
        addr_canonicalize: (sourcePtr: number, destinationPtr: number): number => {
          return doAddrCanonicalize(instance, sourcePtr, destinationPtr);
        },
        addr_humanize: (sourcePtr: number, destinationPtr: number): number => {
          return doAddrHumanize(instance, sourcePtr, destinationPtr);
        },
        secp256k1_verify: (hashPtr: number, signaturePtr: number, pubkeyPtr: number) => {
          return doSecp256k1Verify(instance, hashPtr, signaturePtr, pubkeyPtr);
        },
        secp256k1_recover_pubkey: (hashPtr: number, signaturePtr: number, recoverParam: number): number => {
          return doSecp256k1RecoverPubkey(instance, hashPtr, signaturePtr, recoverParam);
        },
        ed25519_verify: (messagePtr: number, signaturePtr: number, pubkeyPtr: number): number => {
          return doEd25519Verify(instance, messagePtr, signaturePtr, pubkeyPtr);
        },
        ed25519_batch_verify: (messagePtr: number, signaturePtr: number, publicKeysPtr: number): number => {
        ...
        db_scan: (startPtr: number, endPtr: number, order: number): number => {
