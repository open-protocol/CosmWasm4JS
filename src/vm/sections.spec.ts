...
test("decode sections works for multiple elements", () => {
  ...
  dec = decodeSections(
    Buffer.from([
      0xaa, 0x00, 0x00, 0x00, 0x01, 0xde, 0xde, 0x00, 0x00, 0x00, 0x02,
    ])
  );
  expect(dec[0].toString("hex")).toBe("aa");
  ...
  dec = decodeSections(
    Buffer.from([
      0xaa, 0x00, 0x00, 0x00, 0x01, 0xde, 0xde, 0x00, 0x00, 0x00, 0x02, 0x00,
      0x00, 0x00, 0x00,
    ])
  );
  expect(dec[0].toString("hex")).toBe("aa");
  ...
  dec = decodeSections(
    Buffer.from(
      "AA00000001DEDE0000000200000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000013",
      "hex"
    )
  );
  expect(dec[0].toString("hex")).toBe("aa");
